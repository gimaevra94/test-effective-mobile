...
func initRouter(db *database.DB) *chi.Mux {
	...
	r.Get("/api/v1/subscription/{user_id}/{service_name}", handlers.GetSubscription(db))
	r.Patch("/apiv1/subscription/{user_id}/{service_name}", handlers.UpdateSubscription(db))
